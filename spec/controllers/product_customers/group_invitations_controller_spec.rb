    ...
    describe "POST create" do
      ...
      context "with vaild attributes" do
        ...
        describe "when user already invited or member" do
          ...
          let!(:product_customer_user) { create(:product_customer_user, product_customer: product_customer, user: invited_user) }
          let!(:group_invitation_old) { create(:group_invitation, 
            sender: @user, recipient: invited_user, group_invitable: product_customer, email: invited_user.email) }
          subject(:create_action) { post :create, product_customer_id: product_customer.id, group_invitation: attributes_for(:group_invitation, 
            sender: @user, recipient: invited_user, group_invitable: product_customer, email: invited_user.email) } 
        ...
        describe "when user registered but not invited or member" do
          ...
          subject(:create_action) { post :create, product_customer_id: product_customer.id, group_invitation: attributes_for(:group_invitation, 
            sender: @user, recipient: user, group_invitable: product, email: user.email) }
